---
name: miso-ios-components-indicators
description: Usage and code examples for MISO iOS Indicators components — MISOBadgeStandard / MISOBadgeCount / MISOBadgeIcon (statuses, sizes, UInt8 count), MISOTag (label, status, icon, bullet), MISOInputTag (removable), MISOCircularProgressIndicator (determinate / indeterminate, statuses, gap size, track, animated reveal) and MISOLinearProgressIndicator (determinate / indeterminate, statuses, gap size, track, stop indicator, helper text). Load the miso-ios-framework-usage skill first for imports, themes and image rules.
license: MIT
---

# MISO Indicators components
...
```swift
MISOBadgeStandard(accessibilityLabel: "Some label", status: .neutral, size: .medium)
MISOBadgeCount(3, accessibilityLabel: "Some label", status: .neutral, size: .medium)
MISOBadgeIcon(status: .neutral(icon: Image("ic")), accessibilityLabel: "Label", size: .medium)
```
...
```swift
MISOTag(label: "Label")
MISOTag(label: "Label", status: .neutral(icon: MISOImage(asset: Image("ic"))))
MISOTag(label: "Label", status: .neutral(icon: MISOImage(asset: Image("ic"), renderingMode: .original))) // raw image (not tinted)
MISOTag(label: "Label", status: .neutral(icon: MISOImage(asset: Image("ic"), flipped: true))) // flipped for RTL
MISOTag(label: "Label", status: .accent(icon: MISOImage(asset: Image("ic"))))
MISOTag(label: "Label", status: .accent(icon: MISOImage(asset: Image("ic"), renderingMode: .original))) // raw image (not tinted)
...
```swift
MISOInputTag("Label") { /* remove action */ }
```
...
MISOCircularProgressIndicator(status: .info, track: true)

// Custom size — stroke width and gap scale proportionally to the effective size
MISOCircularProgressIndicator(progress: 0.6)
...
- On `MISOColoredSurface`, the indicator switches to a monochrome rendering: `status` is ignored and the surface content color is used.
...
MISOLinearProgressIndicator(progress: 0.5, status: .accent, track: false)
MISOLinearProgressIndicator(progress: 0.3, status: .warning, gapSize: .small)
...
MISOLinearProgressIndicator(progress: 0.5, stopIndicator: true)
...
MISOLinearProgressIndicator(progress: 0.75, helperText: .description("Uploading…", alignment: .start))
...
MISOLinearProgressIndicator(progress: 0.75, helperText: .percent(description: "Uploading…", alignment: .start))
...
MISOLinearProgressIndicator(status: .info, track: true, helperText: "Processing…")
MISOLinearProgressIndicator(status: .accent, gapSize: .small)

// ── Custom width ─────────────────────────────────────────────────────────────

// The bar fills the available horizontal space by default; constrain it via .frame if needed.
MISOLinearProgressIndicator(progress: 0.6)
    .frame(width: 240)
```
